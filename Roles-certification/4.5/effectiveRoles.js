...
(function () {
    ...
    exports.calculateEffectiveRoles = function(object, rolesPropName) {
        ...
        //
        var qry = { "_queryFilter" : "/name eq \"MyDynamicRole\"" };

	MyDynamicRoleId = openidm.query('managed/role',qry,['_id']);
	
	if (MyDynamicRoleId.result.length > 0 && object.accountStatus === 'active') {
	   effectiveRoles = effectiveRoles.concat([
	   {"_ref": "managed/role/" + MyDynamicRoleId.result[0]._id, "name": "MyDynamicRole"}
	             ]);
	 }
